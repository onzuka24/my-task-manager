...
mod tests {
    ...
    fn other_shortcuts_are_ignored() {
        ...
        assert_eq!(
            fold(&[(ShortcutState::Pressed, false)], false),
            (false, 0)
        );
    }
    ...
    fn a_failed_registration_is_a_value_not_an_error() {
        ...
        assert!(!status.registered, "失敗は registered=false として表現される");
        assert_eq!(
        ...
        assert!(
            ...
            "利用者に示す理由を保持すること (Dock もメニューバーも無いため、オーバーレイが唯一の伝達経路)"
        );
    }
